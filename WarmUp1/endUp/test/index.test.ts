import { assert, should } from "chai";
import { endUp } from "../src/index";

/* 
Given a string, return a new string where the last 3 chars are now in upper case. 
If the string has less than 3 chars, uppercase whatever is there. Note that str.toUpperCase() returns the uppercase version of a string.


endUp("Hello") → "HeLLO"
endUp("hi there") → "hi thERE"
endUp("hi") → "HI"
*/

describe("endUp", () => {
  it("Should return 'HeLLO' if given the string parameter 'Hello'", () => {
    const result = endUp("Hello");
    assert.equal(result, "HeLLO");
  });
  it("Should return 'hi thERE' if given the string parameter 'hi there'", () => {
    const result = endUp("hi there");
    assert.equal(result, "hi thERE");
  });
  it("Should return 'HI' if given the string parameter 'hi'", () => {
    const result = endUp("hi");
    assert.equal(result, "HI");
  });
});
